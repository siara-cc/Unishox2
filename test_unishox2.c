...
int unishox2_compress_preset_lines(const char *in, int len, char *out, int preset, struct us_lnk_lst *prev_lines, long *term) {
  switch (preset) {
    case 0:
      return unishox2_compress_lines(in, len, out, USX_PSET_DFLT, prev_lines, term);
    case 1:
      return unishox2_compress_lines(in, len, out, USX_PSET_ALPHA_ONLY, prev_lines, term);
    case 2:
      return unishox2_compress_lines(in, len, out, USX_PSET_ALPHA_NUM_ONLY, prev_lines, term);
    case 3:
      return unishox2_compress_lines(in, len, out, USX_PSET_ALPHA_NUM_SYM_ONLY, prev_lines, term);
    case 4:
      return unishox2_compress_lines(in, len, out, USX_PSET_ALPHA_NUM_SYM_ONLY_TXT, prev_lines, term);
    case 5:
      return unishox2_compress_lines(in, len, out, USX_PSET_FAVOR_ALPHA, prev_lines, term);
    case 6:
      return unishox2_compress_lines(in, len, out, USX_PSET_FAVOR_DICT, prev_lines, term);
    case 7:
      return unishox2_compress_lines(in, len, out, USX_PSET_FAVOR_SYM, prev_lines, term);
    case 8:
      return unishox2_compress_lines(in, len, out, USX_PSET_FAVOR_UMLAUT, prev_lines, term);
    case 9:
      return unishox2_compress_lines(in, len, out, USX_PSET_NO_DICT, prev_lines, term);
    case 10:
      return unishox2_compress_lines(in, len, out, USX_PSET_NO_UNI, prev_lines, term);
    case 11:
      return unishox2_compress_lines(in, len, out, USX_PSET_NO_UNI_FAVOR_TEXT, prev_lines, term);
    case 12:
      return unishox2_compress_lines(in, len, out, USX_PSET_URL, prev_lines, term);
    case 13:
      return unishox2_compress_lines(in, len, out, USX_PSET_JSON, prev_lines, term);
    case 14:
      return unishox2_compress_lines(in, len, out, USX_PSET_JSON_NO_UNI, prev_lines, term);
    case 15:
      return unishox2_compress_lines(in, len, out, USX_PSET_XML, prev_lines, term);
    case 16:
      return unishox2_compress_lines(in, len, out, USX_PSET_HTML, prev_lines, term);
  }
  return 0;
}
...
int test_ushx_cd(char *input, int preset) {
  ...
  dlen = unishox2_decompress_preset_lines(cbuf, sizeof cbuf, dbuf, preset, NULL);
  dbuf[dlen] = '\0';
  if (dlen != len) {
...
if (argv >= 4 && strcmp(args[1], "-c") == 0) {
   ...
   do {
     ...
     if (bytes_read > 0) {
        clen = unishox2_compress_preset_lines(cbuf, bytes_read, dbuf, preset, NULL, NULL);
        ctot += clen;
        tot_len += bytes_read;
        if (clen > 0) {
           fputc(clen >> 8, wfp);
...
if (argv >= 4 && (strcmp(args[1], "-g") == 0 || 
   ...
   while (fgets(cbuf, sizeof(cbuf), fp) != NULL) {
      ...
      if (len > 0) {
        ...
        clen = unishox2_compress_preset_lines(cbuf, len, dbuf, preset, cur_line, NULL);
        if (clen > 0) {
            perc = (float)(len-clen);
            perc /= len;
            perc *= 100;
...
if (argv >= 2 && strcmp(args[1], "-t") == 0) {
   ...
   // check template
   {
     ...
     const int clen = unishox2_compress_lines(hex, len, cbuf, USX_HCODES_DFLT, USX_HCODE_LENS_DFLT, USX_FREQ_SEQ_DFLT, (const char *[]){":FFFFFF", "-ffffff", 0, 0, 0} , NULL, NULL);
...
if (argv == 2 || (argv == 3 && atoi(args[2]) > 0)) {
   ...
   ctot = unishox2_compress_preset_lines(args[1], len, cbuf, preset, NULL, NULL);
